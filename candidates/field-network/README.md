# Field Network Format Candidate

## Summary

This candidate represents magic through sources, sinks, transforms, samplers, and spatial or abstract fields. It is intended to be serializable, procedurally generatable, interpretable into performer-facing data, and usable by games ranging from simple rule worlds to detailed physical simulations.

## Core concepts

- **Spell**: one declarative unit in the sources, sinks, transforms, samplers, and spatial or abstract fields model, with typed attributes, requirements, validation bounds, metadata, and extension data.
- **Magic**: the complete sources, sinks, transforms, samplers, and spatial or abstract fields composition containing many spells, entry points, policies, spell provenance summaries, and diagnostics.
...
- **JSON representation**: a readable object containing the sources, sinks, transforms, samplers, and spatial or abstract fields topology, spell structures, magic structure, requirements, policies, spell provenance summaries, and metadata.
...
- **Interpreter output**: a `MagicFieldNetwork` structure containing resolved spells, candidate topology, lookup tables, typed attributes, requirements, spell provenance summaries, extension payloads, and diagnostics.
...
```json
{
  ...
  "entry_point": "thermal_lift_field",
  "sources": [
    {
      "handle": "thermal_lift_source",
      "spell": 0
    }
  ],
  "transforms": [
    {
      "from": "thermal_lift_source",
      "to": "thermal_lift_field",
      "operation": "resolve_thermal_lift"
    }
  ],
  "sinks": [
    {
      "from": "thermal_lift_field",
      "target": "selected_volume"
    }
  ]
}
...
The spell selects a target volume, adds bounded heat, converts the heated air into upward force, and removes the force after the declared duration. In this candidate, the magic composes the spell through local field source, transform, and sink handles, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "entry_point": "water_wall_field",
  "sources": [
    {
      "handle": "water_wall_source",
      "spell": 0
    }
  ],
  "transforms": [
    {
      "from": "water_wall_source",
      "to": "water_wall_field",
      "operation": "resolve_water_wall"
    }
  ],
  "sinks": [
    {
      "from": "water_wall_field",
...
The spell gathers available water or water-tagged entities, shapes them into a bounded barrier, increases cohesion, and releases the water when the duration expires. In this candidate, the magic composes the spell through local field source, transform, and sink handles, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "entry_point": "stone_brace_field",
  "sources": [
    {
      "handle": "stone_brace_source",
      "spell": 0
    }
  ],
  "transforms": [
    {
      "from": "stone_brace_source",
      "to": "stone_brace_field",
      "operation": "resolve_stone_brace"
    }
  ],
  "sinks": [
    {
      "from": "stone_brace_field",
      "target": "touched_structure"
    }
  ]
}
...
The spell reinforces a touched stone or structure by bounded stiffness and fracture modifiers, records the original material state, and restores it after expiry. In this candidate, the magic composes the spell through local field source, transform, and sink handles, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "entry_point": "gravity_snare_field",
  "sources": [
    {
      "handle": "gravity_snare_source",
      "spell": 0
    }
  ],
  "transforms": [
    {
      "from": "gravity_snare_source",
      "to": "gravity_snare_field",
      "operation": "resolve_gravity_snare"
    }
  ],
  "sinks": [
    {
      "from": "gravity_snare_field",
      "target": "selected_area"
    }
  ]
}
...
The spell creates a local gravity field around an anchor, filters excluded targets, clamps pull strength and target count, and applies temporary inward force. In this candidate, the magic composes the spell through local field source, transform, and sink handles, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "entry_point": "soft_repair_field",
  "sources": [
    {
      "handle": "soft_repair_source",
      "spell": 0
    }
  ],
  "transforms": [
    {
      "from": "soft_repair_source",
      "to": "soft_repair_field",
      "operation": "resolve_soft_repair"
    }
  ],
  "sinks": [
    {
      "from": "soft_repair_field",
      "target": "damaged_soft_material"
    }
  ]
}
...
The spell repairs damaged soft or living material gradually, spends bounded energy, and stops before exceeding the declared integrity threshold. In this candidate, the magic composes the spell through local field source, transform, and sink handles, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
