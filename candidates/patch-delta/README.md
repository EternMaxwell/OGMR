...
- **Magic**: the complete declarative patches that add, replace, scale, or remove world properties composition containing many spells, entry points, policies, provenance, and diagnostics.
...
- **JSON representation**: a readable object containing the declarative patches that add, replace, scale, or remove world properties topology, spell structures, magic structure, requirements, policies, provenance, and metadata.
...
- **Interpreter output**: a `MagicPatchDelta` structure containing resolved spells, candidate topology, lookup tables, typed attributes, requirements, generation provenance, extension payloads, and diagnostics.
