# Patch Delta Format Candidate

## Summary

This candidate represents magic through declarative patches that add, replace, scale, or remove world properties. It is intended to be serializable, procedurally generatable, interpretable into performer-facing data, and usable by games ranging from simple rule worlds to detailed physical simulations.

## Core concepts

- **Spell**: one declarative unit in the declarative patches that add, replace, scale, or remove world properties model, with typed attributes, requirements, validation bounds, metadata, and extension data.
- **Magic**: the complete declarative patches that add, replace, scale, or remove world properties composition containing many spells, entry points, policies, spell provenance summaries, and diagnostics.
...
- **JSON representation**: a readable object containing the declarative patches that add, replace, scale, or remove world properties topology, spell structures, magic structure, requirements, policies, spell provenance summaries, and metadata.
...
- **Interpreter output**: a `MagicPatchDelta` structure containing resolved spells, candidate topology, lookup tables, typed attributes, requirements, spell provenance summaries, extension payloads, and diagnostics.
...
```json
{
  ...
  "patches": [
    {
      "spell": 0,
      "path": "target.effects.thermal_lift",
      "op": "add",
      "value": true
    }
  ],
  "rollback": "after_duration"
}
```
...
```json
{
  ...
  "patches": [
    {
      "spell": 0,
      "path": "target.effects.water_wall",
      "op": "add",
      "value": true
    }
  ],
...
The spell gathers available water or water-tagged entities, shapes them into a bounded barrier, increases cohesion, and releases the water when the duration expires. In this candidate, the magic composes the spell through patch operations with rollback, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "patches": [
    {
      "spell": 0,
      "path": "target.effects.stone_brace",
      "op": "add",
      "value": true
    }
  ],
...
The spell reinforces a touched stone or structure by bounded stiffness and fracture modifiers, records the original material state, and restores it after expiry. In this candidate, the magic composes the spell through patch operations with rollback, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "patches": [
    {
      "spell": 0,
      "path": "target.effects.gravity_snare",
      "op": "add",
      "value": true
    }
  ],
...
The spell creates a local gravity field around an anchor, filters excluded targets, clamps pull strength and target count, and applies temporary inward force. In this candidate, the magic composes the spell through patch operations with rollback, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "patches": [
    {
      "spell": 0,
      "path": "target.effects.soft_repair",
      "op": "add",
      "value": true
    }
  ],
...
The spell repairs damaged soft or living material gradually, spends bounded energy, and stops before exceeding the declared integrity threshold. In this candidate, the magic composes the spell through patch operations with rollback, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
