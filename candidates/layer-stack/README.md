# Layer Stack Format Candidate

## Summary

This candidate represents magic through ordered layers, masks, blends, overrides, and accumulators. It is intended to be serializable, procedurally generatable, interpretable into performer-facing data, and usable by games ranging from simple rule worlds to detailed physical simulations.

## Core concepts

- **Spell**: one declarative unit in the ordered layers, masks, blends, overrides, and accumulators model, with typed attributes, requirements, validation bounds, metadata, and extension data.
- **Magic**: the complete ordered layers, masks, blends, overrides, and accumulators composition containing many spells, entry points, policies, spell provenance summaries, and diagnostics.
...
- **JSON representation**: a readable object containing the ordered layers, masks, blends, overrides, and accumulators topology, spell structures, magic structure, requirements, policies, spell provenance summaries, and metadata.
...
- **Interpreter output**: a `MagicLayerStack` structure containing resolved spells, candidate topology, lookup tables, typed attributes, requirements, spell provenance summaries, extension payloads, and diagnostics.
...
```json
{
  ...
  "entry_point": "layer_stack",
  "layers": [
    {
      "spell": 0,
      "mask": "selected_volume",
      "blend": "apply_thermal_lift"
    }
  ],
  "accumulators": [
    "thermal_lift_result"
...
The spell selects a target volume, adds bounded heat, converts the heated air into upward force, and removes the force after the declared duration. In this candidate, the magic composes the spell through a masked layer in the stack, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
The spell gathers available water or water-tagged entities, shapes them into a bounded barrier, increases cohesion, and releases the water when the duration expires. In this candidate, the magic composes the spell through a masked layer in the stack, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "layers": [
    {
      "spell": 0,
      "mask": "touched_structure",
      "blend": "apply_stone_brace"
    }
  ],
  "accumulators": [
    "stone_brace_result"
...
The spell reinforces a touched stone or structure by bounded stiffness and fracture modifiers, records the original material state, and restores it after expiry. In this candidate, the magic composes the spell through a masked layer in the stack, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
  ...
  "layers": [
    {
      "spell": 0,
      "mask": "selected_area",
      "blend": "apply_gravity_snare"
    }
  ],
  "accumulators": [
    "gravity_snare_result"
  ]
}
```
...
The spell creates a local gravity field around an anchor, filters excluded targets, clamps pull strength and target count, and applies temporary inward force. In this candidate, the magic composes the spell through a masked layer in the stack, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
...
```json
{
  ...
  "layers": [
    {
      "spell": 0,
      "mask": "damaged_soft_material",
      "blend": "apply_soft_repair"
    }
  ],
  "accumulators": [
    "soft_repair_result"
...
The spell repairs damaged soft or living material gradually, spends bounded energy, and stops before exceeding the declared integrity threshold. In this candidate, the magic composes the spell through a masked layer in the stack, so validation can prove the performer has the required capabilities, bounded parameters, and resolvable local structure before runtime effects are applied.
